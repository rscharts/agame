...
$(document).ready(function() {
	...
	function buyWorker() {
		...
		if ((workerCount + 1) <= ( Math.floor(workers[workerType].limit * maxWorkerMultiplier) )) {
			...
			popup('WHOOPS!', 'You can not buy any more workers of that type.', '', 0);
		}
	...
	function buyMaxWorkers() {
		...
		for (i; money >= totalcost; i++) {
			...
			if ( (money <= totalcost + newAmount) || (newWorkerCount >= Math.floor(workers[workerType].limit * maxWorkerMultiplier) ))
				break;
	...
	function sellWorker() {
		...
		if (employed[workerType][0] > 0) {
			...
			popup('WHOOPS!', 'You can\'t sell something you don\'t have.', '', 0);
		}
	}
	...
	$(document).on('click', 'table[name="workers"] button[name="pay_wages"]', payWages);
