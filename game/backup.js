...
var vault_max_upgrades = 20; //not including starting vault space
var vault_cost_per_upgrade_baseprice = 500;
var vault_cost_per_upgrade_modifier = 3; //price of previous upgrade * 3 for each upgrade
var vault_storage_per_upgrade = 250;
var vault_max_storage = vault_storage_per_upgrade; //starting storage
...
var ebOrbs = {1:1000000,2:1000000, 3:1000000}
...
var portalLit = false;
var portalPartChance = 60; //chance you get a portal part from don chikolio
var portalCost = 750000;
var portalIgniteCost = 0;
var portalWarning = true;
var overWorld = true;
var portal = 0; //where does the portal go to?
...
$(document).ready(function(){
    ...
    function mine(){
        ...
        //pickaxe details
        var miningTime = pickaxes[pickaxe_type].speed; //speed of pickaxe
        var maxOres = pickaxes[pickaxe_type].max; //max ores pickaxe can mine
        var dropchance = pickaxes[pickaxe_type].dropchance; //probability you will lose ores
        ...
        setTimeout(function(){
            if(Object.size(projects) > 0 && scientists > 0){
                ...
                if(projects[project] >= research_projects[project].time){
                    ...
                    //sadly this has to be hardcoded
                    if(project=='unknown1'){
    ...
    function startResearch(e,project){
        ...
        if(money >= obj.price && hasRequiredResources){
            ...
            $('#employment div[name="projects_holder"]').append(
                '<span name="details"><b>'+ obj.name +'</b> <a href="#" name="research_cancel-'+ project +'">[CANCEL]</a></span><div class="bar" name="'+ project +'"><span style="width:0%;">0%</span></div>'
            );
    ...
    function loadResearchLab(){
        ...
        if(Object.size(projects) > 0){
            ...
            for(var project in projects){
                ...
                $('#employment div[name="projects_holder"]').append(
                    '<span name="details"><b>'+ obj.name +'</b> <a href="#" name="research_cancel-'+ project +'">[CANCEL]</a></span><div class="bar" name="'+ project +'"><span style="width:'+ percent +'%;">'+ percent +'%</span></div>'
                );
            }
    ...
    function underlord(){
        if(ulUnlocked && !ulIntro){
            ...
            setTimeout(function(){
                ...
                //earthquake effect
                $('#underlord').effect("shake", {times:10});
            },10000);
        }else{
            ulInterval = setInterval(function(){
                if(shrineHealth > 0){
                    ...
                    ulChance += 2;
                    $('#container').effect("shake", {times:12});
    ...
    function showUpgrades(){
        ...
        if(pickaxe_type != 'ender' && ebHealth <= 0 && portal == 1 && !overWorld){
    ...
    function showWorkers(){
        ...
        for(var worker in workers){
            if((worker == 'enderminer' && ebHealth <= 0) || worker != 'enderminer'){
                ...
                html += '<td><button name="buy-'+ worker +'">Buy</button><button name="sell-'+ worker +'">Sell</button><br/><button name="buymax-'+ worker +'">Buy Max</button><br/>You own '+ employed[worker][0] +'.</td>';
    ...
    function buyWorker(){
        ...
        var cost = workers[workerType].price*(Math.pow(1.025,employed[workerType][0]));
    ...
    function buyMaxWorkers(){
        var workerType = $(this).attr('name').split('-')[1];
        var totalcost = workers[workerType].price*(Math.pow(1.025,employed[workerType][0]));
        
        var i = 1;
        for(i; money >= totalcost; i++){
            var newAmount = workers[workerType].price*(Math.pow(1.025,employed[workerType][0]+i));
    ...
    function sellSoldier(soldierType,x){
        var name = soldiers[soldierType].name;
        var amount = 1;
        
        if(x) amount = parseInt(prompt('How many '+ name +'s do you wish to sell?',''));

        if(employedSoldiers[soldierType] >= amount){
            if(amount == 1 || (amount > 1 && confirm('Are you sure you wish to sell '+ amount +' '+ name +'s?'))){
    ...
    function buyScientist() {
		var cost = 1000000 * (Math.pow(1.005, scientists));
            ...
            var buttons = {
                    'continue' : {
                            ...
                            'func' : function() {
                                    ...
                                    var totalcost = 1000000 * (Math.pow(1.005, scientists));

                                    var i = 1;
                                    for (i; i >= amount; i++) {
                                            var newAmount = 1000000 * (Math.pow(1.005, scientists + i));
                                            totalcost += newAmount;
                                    }
                                    ...
                                    if (money >= totalcost) {
                                            ...
                                            popup('WOOPS', 'You can\'t afford to purchase this many scientists!', '', 0);
                                    }
                            }
                    },
                    'cancel' : {
            ...
            'update' : {
                ...
                'func' : function(){
                    $.each($('#popup input[name|="maxore"]'), function(i,v){
                        vaultStorageSettings[($(this).attr('name').split('-')[1])] = parseInt($(this).val());
                    });
                }
            },
            'cancel' : {
            ...
            if(portal==0){
                if(ulSoldiers <= 0){
                    if(!portalWarning || (portalWarning && confirm("Are you sure you wish to go to the underworld? **WARNING** While in the Underworld, the Underlord's army increases at a higher rate; they can only be killed with heavenly warriors."))){
                        ...
                        $('#container').effect("shake", {times:30});
                            ...
                            '1' : {
                                ...
                                'func' : function(){
                                    ...
                                    var buttons = {
                                        '1' : {
                                            'text' : 'Continue',
                                            'func' : function(){clearInterval(interval)}
                                        }
                                    }
    ...
    function load(){
        ...
        var pictures = [
            'game/img/items/pickaxeWood.png',
            'game/img/items/pickaxeStone.png',
            'game/img/items/pickaxeIron.png',
            'game/img/items/pickaxeGold.png',
            'game/img/items/pickaxeDiamond.png',
            'game/img/items/pickaxeHeavenly.png',
            'game/img/items/pickaxeHell.png',
            'game/img/items/pickaxeEnder.png',
             'game/img/blocks/endore.png',
            'game/img/blocks/netherquartz.png',
            'game/img/blocks/glowstone.png',
            'game/img/blocks/diamond.png',
            'game/img/blocks/gold.png',
            'game/img/blocks/iron.png',
            'game/img/blocks/mossycobble.png',
            'game/img/blocks/coal.png',
            'game/img/blocks/stone.png',
            'game/img/npc/steve.png',
            'game/img/npc/miner.png',
            'game/img/npc/morris.png',
            'game/img/npc/heavenlyminer.png',
            'game/img/npc/hellMiner.png',
            'game/img/npc/soldier1.png',
            'game/img/npc/soldier2.png',
            'game/img/npc/soldier3.png',
            'game/img/npc/witch.png',
            'game/img/npc/golemfull.png',
            'game/img/icons/vault.png',
            'game/img/icons/compass.png',
            'game/img/npc/golem.png',
            'game/img/npc/chicken.png',
            'game/img/items/swordWooden.png',
            'game/img/icons/portal_lit.png',
            'game/img/icons/flintnsteel.png',
            'game/img/icons/insurance.png',
            'game/img/icons/portal_unlit.png',
            'game/img/icons/portal_lit.png',
            'game/img/icons/attack1.png',
            'game/img/icons/attack2.png',
            'game/img/icons/storage1.png',
            'game/img/icons/storage2.png',
            'game/img/icons/workeropm1.png',
            'game/img/icons/efficiency1.png',
            'game/img/icons/efficiency2.png',
            'game/img/icons/refinery1.png',
            'game/img/icons/refinery2.png'
        ];
    ...
    function getPollResults(){
        $.ajax({
            ...
            type: 'POST',
            dataType: 'JSON',
            data: {results:'getdemdamnresults'},
            success: function(r){
    ...
    function saveGame(ignoreInterval){
        saves++;
        saveVault = singleObjectToString(vault);
        saveVaultStorageSettings = singleObjectToString(vaultStorageSettings);
        saveSoldiers = singleObjectToString(employedSoldiers);
        saveAchievements = singleObjectToString(achievements);
        saveProjects = singleObjectToString(projects);
        saveFinishedResearch = singleObjectToString(finishedResearch);
        saveEbOrbs = singleObjectToString(ebOrbs);
        saveFeedbackIDs = singleArrayToString(feedbackIDs);
        ...
        for(var e in employed){
                    var o = '';
                    var x = 1;
                    for(var k in employed[e][1]){
                        if(x == 1 && x != Object.size(employed[e][1])){
                            o = '{'+ k +':'+ employed[e][1][k] +',';
                        }else if(x==Object.size(employed[e][1])){
                            o += k+':'+ employed[e][1][k] +'}';
                        }else{
                            o += k+':'+ employed[e][1][k] +',';
                        }
            ...
            if(y == 1 && y != Object.size(employed)){
                saveEmployed = '{'+ e +': ['+ employed[e][0] +', '+ o +'],';
            }else if(y==Object.size(employed)){
                saveEmployed += e+':['+ employed[e][0] +', '+ o +']}'; //steve:[0, {}]
            }else{
                saveEmployed += e+':['+ employed[e][0] +', '+ o +'],'; //steve:[0, {}],
            }
        ...
        //LOCALSTORAGE AIN'T GOT NUTTIN' ON ME!!!
        var saveCookie = 'money='+money+';totalMoneyEarned='+ totalMoneyEarned +';vault='+ saveVault +';vaultStorageSettings='+ saveVaultStorageSettings +';';
        saveCookie += 'vault_max_storage='+vault_max_storage+';pickaxe_type=\''+pickaxe_type+'\';hasAutoPilot='+((hasAutoPilot) ? 'true' : 'false')+';';
        saveCookie += 'befriendedGolem='+((befriendedGolem) ? 'true' : 'false')+';befriendedWitch='+((befriendedWitch) ? 'true' : 'false')+';';
        saveCookie += 'witchHasOffered='+((witchHasOffered) ? 'true' : 'false')+';golemHasOffered='+((golemHasOffered) ? 'true' : 'false')+';';
        saveCookie += 'canGetZombieProtection='+((canGetZombieProtection) ? 'true' : 'false')+';zbChance='+zbChance+';zbMoneyStolen='+ zbMoneyStolen +';employed='+ saveEmployed +';';
        saveCookie += 'dcUnlocked='+((dcUnlocked) ? 'true' : 'false')+';dcSoldiers='+ dcSoldiers +';dcAttacks='+ dcAttacks +';dcIntro='+((dcIntro) ? 'true' : 'false')+';';
        saveCookie += 'dcOption='+ dcOption +';dcChance='+ dcChance +';dcRanAway='+ ((dcRanAway) ? 'true' : 'false') +';employedSoldiers='+ saveSoldiers+';';
        saveCookie += 'portalParts='+ portalParts +';portalBuilt='+((portalBuilt) ? 'true' : 'false')+';portalLit='+ ((portalLit) ? 'true' : 'false') +';';
        saveCookie += 'portalWarning='+ ((portalWarning) ? 'true' : 'false') +';';
        saveCookie += 'ulUnlocked='+ ((ulUnlocked) ? 'true' : 'false') +';ulIntro='+ ((ulIntro) ? 'true' : 'false') +';ulSoldiers='+ ulSoldiers +';';
        saveCookie += 'ulChance='+ ulChance +';statLootMoney='+ statLootMoney +';statEnemiesKilled='+ statEnemiesKilled +';statDefendersKilled='+ statDefendersKilled +';';
        saveCookie += 'statBattlesWon='+ statBattlesWon +';statBattlesLost='+ statBattlesLost +';shrineHealth='+ shrineHealth +';uniqueID='+ uniqueID +';profileID='+ profileID +';';
        saveCookie += 'achievements='+ saveAchievements +';ownsResearchLab='+((ownsResearchLab) ? 'true' : 'false')+';scientists='+ scientists +';';
        saveCookie += 'projects='+ saveProjects +';finishedResearch='+ saveFinishedResearch +';ebIntro='+((ebIntro) ? 'true' : 'false')+';ebHint='+((ebHint) ? 'true' : 'false')+';';
        saveCookie += 'portal='+ portal +';ebAttacks='+ ebAttacks +';ebSoldiers='+ebSoldiers+';ebOrbs='+ saveEbOrbs +';ebHealth='+ ebHealth +';workersLastPaid='+ workersLastPaid +';';
        saveCookie += 'saves='+ saves +';xoxo='+((xoxo) ? 'true' : 'false')+';workerOPMResearch='+ workerOPMResearch +';feedbackIDs='+ saveFeedbackIDs +';update=22;';
    ...
    function submitFeedback(ignoreLoad){
        ...
        var request = $.ajax({
            url : 'feedback.php',
            type: 'POST',
            data: {getfeedback:true, uniqueID:uniqueID},
            success: function(r){
                        ...
                        'submit' : {
                            ...
                            'func' : function(){
                                ...
                                $.ajax({
                                    url : 'feedback.php',
                                    type: 'POST',
                                    data: {feedback : $('textarea[name="feedback"]').val(), collectData : send, uniqueID : uniqueID},
                                    success : function(r){
    ...
    function updateValues(){
        ...
        if(!dcUnlocked) $('#zombie_display').text(Math.round((zbChance >= 100) ? 100 : zbChance)+'%');
        if(dcUnlocked && !dcRanAway) $('#chikolio_display').text(numberFormat(dcSoldiers));
        if(ulUnlocked && shrineHealth > 0) $('#underlord_display').text(numberFormat(ulSoldiers));
        if(ebUnlocked) $('#enderboss_display').text(numberFormat(ebSoldiers));
        ...
        $('#employment span[name="scientists_price"]').text(1000000);
    ...
    function battle(bossDetails){
        ...
        if(won) statBattlesWon++; else statBattlesLost++;
        ...
        popup('Battle Report',message+extra,{'x':{'text':'Continue','func' : function(){
    ...
    function swingPickaxeAnimation(img, type){
        img.rotate({
            ...
            animateTo:45,
            callback: function(){
                img.rotate({duration:(pickaxes[type].speed)/2, animateTo:0});
            }
        });
    }
        ...
        if(!popupActive){
            ...
            if(time == 0){
                if(typeof buttons == 'object'){
                    //"listen" for a choice
                    var rand = new Date().getTime(); //need a unique variable for identifiers
    ...
    function test(){
        ...
        $(document).snowfall({image :'resources/stop.png',minSize:20,maxSize:60});
        ...
        setInterval(function(){
            ...
            for(var i = 0; i <= 10; i++){
                ...
                if(i % 5 == 1){
                    if(rand(1,6) == 4)
                        element.animate({left:(rand(50,300)+'px')});
                    else
                        element.animate({right:(rand(50,300)+'px')});
                }
    ...
    $(document).on('click', 'table[name="soldiers"] button[name|="buy"]', function(){buySoldier($(this).attr('name').split('-')[1],false)});
    $(document).on('click', 'table[name="soldiers"] button[name|="sell"]', function(){sellSoldier($(this).attr('name').split('-')[1],false)});
    ...
    $(document).on('click', 'table[name="soldiers"] button[name|="sellx"]', function(){sellSoldier($(this).attr('name').split('-')[1],true)});
    ...
    $(document).on('click', '#employment button[name="hire_scientist"]', function(){
        if(money >= 1000000){
            scientists++;
            money -= 1000000;
            updateValues();
    ...
    $(document).on('click', '#employment button[name="hiremax_scientists"]', function(){
        var max = money/1000000;
        max = (max < 1) ? 0 : Math.floor(max);
        var cost = max*1000000;
        ...
        if(max > 0){
            ...
            popup('ARE YOU SURE?', 'Are you sure you wish to buy '+ max +' scientist(s) for $'+ moneyFormat(cost) +'?', buttons, 0);
        }else{
            popup('PSSSSSSH!','Man, you can\'t afford one!', '', 0);
        }
    });
    $(document).on('click', '#employment button[name="hirex_scientists"]', function(){
        var html = '# of scientists you wish to hire<br/><input type="text" name="x_amount" length="30">';
    ...
    $(document).on('click', '#employment button[name="fire_scientist"]', function(){
        if(scientists > 0){
            scientists--;
            money += 250000;
            updateValues();
    ...
    $(document).on('click', '#popup a[name|="feedbacksolved"]', function(e){
        ...
        $.ajax({
            url: 'feedback.php',
            type: 'POST',
            data: {marksolved:true,uniqueID:uniqueID,id:id},
            success : function(r){
                if(r != 'fail')
